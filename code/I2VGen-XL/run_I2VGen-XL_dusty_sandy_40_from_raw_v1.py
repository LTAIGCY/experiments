...
OUT_DIR = "/root/autodl-tmp/datasets/gen_videos_pilot/baselines/I2VGen-XL/dusty_sandy_3s_from_raw_v1"
...
NEG_PROMPT = (
    "rain, wet road, muddy road, snow, heavy fog, clear asphalt road, "
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "I2VGen-XL_dusty_sandy_prompt_used.txt"), "w", encoding="utf-8") as f:
        f.write("PROMPT:\n")
        f.write(PROMPT)
        f.write("\n\nNEG_PROMPT:\n")
        f.write(NEG_PROMPT)
    ...
    for idx, img_path in enumerate(input_images):
        name = f"{idx:03d}"
        temp_mp4 = os.path.join(OUT_DIR, f"__temp_I2VGen-XL_dusty_sandy_{name}.mp4")
        final_mp4 = os.path.join(OUT_DIR, f"I2VGen-XL_dusty_sandy_{name}_3s.mp4")
    ...
    print("=" * 80)
    print("All I2VGen-XL dusty_sandy videos finished.")
