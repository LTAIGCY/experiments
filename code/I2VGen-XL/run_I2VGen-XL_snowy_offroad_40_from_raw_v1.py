...
OUT_DIR = "/root/autodl-tmp/datasets/gen_videos_pilot/baselines/I2VGen-XL/snowy_offroad_3s_from_raw_v1"

PROMPT = (
    "realistic first-person off-road driving video, snowy off-road winter condition, "
    "compacted snow on the road, shallow tire tracks, partially exposed dirt texture, "
    "snow-covered grass, bushes and tree branches, cloudy overcast winter lighting, "
    "low-speed stable forward camera motion, preserve original road layout and terrain, photorealistic"
)

NEG_PROMPT = (
    "clear sky, sunny weather, blue sky, dry road, muddy road, heavy rain, "
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "I2VGen-XL_snowy_offroad_prompt_used.txt"), "w", encoding="utf-8") as f:
        f.write("PROMPT:\n")
        f.write(PROMPT)
        f.write("\n\nNEG_PROMPT:\n")
        f.write(NEG_PROMPT)
    ...
    for idx, img_path in enumerate(input_images):
        name = f"{idx:03d}"
        temp_mp4 = os.path.join(OUT_DIR, f"__temp_I2VGen-XL_snowy_offroad_{name}.mp4")
        final_mp4 = os.path.join(OUT_DIR, f"I2VGen-XL_snowy_offroad_{name}_3s.mp4")
    ...
    print("=" * 80)
    print("All I2VGen-XL snowy_offroad videos finished.")
