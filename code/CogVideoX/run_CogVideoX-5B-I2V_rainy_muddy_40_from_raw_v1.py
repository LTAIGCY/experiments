...
OUT_DIR = "/root/autodl-tmp/datasets/gen_videos_pilot/baselines/CogVideoX-5B-I2V/rainy_muddy_3s_from_raw_v1"

PROMPT = (
    "realistic first-person off-road driving video in rainy muddy condition, "
    "wet muddy dirt road, shallow puddles, muddy tire tracks, dark wet soil, "
    "damp roadside vegetation, cloudy overcast lighting, stable forward camera motion, "
    "preserve original road layout and terrain, no vehicle, no people, photorealistic"
)
...
def main():
    ...
    with open(os.path.join(OUT_DIR, "CogVideoX-5B-I2V_rainy_muddy_prompt_used.txt"), "w", encoding="utf-8") as f:
        f.write(PROMPT)
    ...
    for idx, img_path in enumerate(input_images):
        name = f"{idx:03d}"
        temp_mp4 = os.path.join(OUT_DIR, f"__temp_CogVideoX-5B-I2V_rainy_muddy_{name}.mp4")
        final_mp4 = os.path.join(OUT_DIR, f"CogVideoX-5B-I2V_rainy_muddy_{name}_3s.mp4")
    ...
    print("=" * 80)
    print("All CogVideoX-5B-I2V rainy_muddy videos finished.")
